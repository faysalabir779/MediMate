...
@Composable
fun DeclinedOrder(allViewModel: AllViewModel, applicationContext: Context) {
    val savedData by allViewModel.preferenceData.collectAsState()

    var hasDeclinedOrder by remember { mutableStateOf(false) }
    ...
    Column {
        ...
        if (!hasDeclinedOrder){
            EmptyMessage(
                orderStatus = "Declined Order",
                painter = painterResource(id = R.drawable.emoticon),
                note = stringResource(
                    id = R.string.declined_text
                )
            )
        }
    }
...
@Composable
fun DeclinedOrder(item: GetAllOrderDetailsItem, allViewModel: AllViewModel) {
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .height(140.dp)
